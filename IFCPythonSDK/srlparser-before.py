    ...
    for entity in entities:
        ...
        if entity.attr:#has attr
            for attr in entity.attr:
                extractAttr(attr,ids)
            subids.update(ids)
        else:
            subids.update(extractSubItems(ids))

def extractAttr(attr,ids):
    ...
    if attr.attr: #has sub attr
        for attr in attr.attr:
        #get every entity's attr ref id ->4,5,6
            extractAttr(attr,refs)
        subids.update(refs)
    else:#append
        subids.update(extractSubItems(refs))
...
def generateModelFile(ids,to=None):
    """"""
    code=''
    lines=getLines(ids)
    for line in lines:
